# Array

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


数组是类似列表的对象，其原型具有执行遍历和变异操作的方法

JavaScript 数组的长度和元素的类型都不固定

由于数组的长度可以随时改变，并且数据可以存储在数组中的非连续位置，因此不能保证 JavaScript 数组密集( 有可能是稀数组 )

数组可以使用非数字的 key, 但用该方法添加的属性不会计算进数组的长度中


//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -//


对象用于在单个变量中存储多个值

数组索引从零开始：数组中的第一个元素的索引为 0，第二个元素的索引为 1，依此类推

数组中可以存储不同类型的数据: Number、String、Boolean、Null、undefined、new object


//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -//


// 数组 和 对象 之间的区别


在 JavaScript 中，`数组` 使用 `编号索引` 

在 JavaScript 中，`对象` 使用 `命名索引`


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

## Basic Syntax

#### Create Array

``` javascript
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


// 构造函数方式创建数组

var arrayName = new Array(arrayLength | ...item);                         


// 空数组

var arr = new Array(): undefined;


// 定义数组，并且设置数组长度                           // 设置数组长度可能会截断数组

var arr = new Array(length: number);                // a single param


// 定义数组, 并设置多个数组项( 数组会自动计算长度 )

var arr = new Array( "item1", "itetm2" ... )        // multi-params


//--------------------------------------------------------------------------------------------------------------------//


// 字面亮方式创建数组

var arrayName = [];                                  // 建议使用


//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -//


// 空数组

var cars = []: undefined;


//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -//


// 数组长度                                          // 避免直接设置数组长度

var arr = ["Saab", "Volvo", "BMW"];

console.log(arr.length);                            // 与 new Array(Length); 不同


//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -//

// 删除数组中的某项                                   // 不建议使用( 该做法会导致稀松数组: 索引不连续 )

delete arr[index: number];                          // 删除指定项后，当前项为空
arr.splice(start: number, count: number): boolean;  // 


// 清空数组

arr.splice(0, arr.lenght);

arr.lenght = 0;


// 查询指定数组项，并返回指定数组项对应的索引

arr.indexOf();                                      // 返回指定数组项第一次出现时所对应的索引

arr.lastIndexOf();                                  // 返回指定数组项最后一次出现时所对应的索引


//-------------------------------------------------------------------------------------------------------------------//

/**
 * 稀松数组
 * 
 * 
 * 单独添加逗号不报错，也会分割为多个，返回 undefined
 */

var arr  = [,];                                     // [undefined x 1]
var arr  = [,,];                                    // [undefined x 2]


//--------------------------------------------------------------------------------------------------------------------//


// 传入多位功能相同

var arr = [1, 2, 3, , , 6];                         // arr.length = 7;

var arr = new Array(1, 2, 3, , , 6)                 // arr.length = 7;


//--------------------------------------------------------------------------------------------------------------------//


// 区别: 单独传一位功能不同

var arr = [10];                                     // [10];

var arr = new Array(10);                            // arr.length = 10;


//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //


/**
 * new Array 不能传入小数，会报错
 * 
 * 
 * new Array(?arrayLength: number)
 */

var arr = [10];                                     // [10.2];

var arr = new Array(10.2);                          // Invalid array length


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### Get item of Array

通过引用索引号来访问数组元素

> 注意：数组索引从0开始。[0]是第一个元素。[1]是第二个元素

``` javascript
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


var car = ["Saab", "Volvo", "BMW"];

// Get the value of an array element

car['0'];       car[0];         // "Saab"
car['1'];       car[1];         // "Volvo"
car['2'];       car[2];         // "BMW"


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### Change item of Array

``` javascript
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


var car = ["Saab", "Volvo", "BMW"];

cars[0] = "Opel";


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### Invoking Array

``` javascript
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


/**
 * 通过引用数组名称来访问整个数组
 * 
 * 
 * 
 */

var cars = ["Saab", "Volvo", "BMW"];

document.getElementById("demo").innerHTML = cars;


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### Array Length

``` javascript
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


/**
 * 数组长度为数组子项总数引减 1
 * 
 * 
 * 
 */

const cars = ["Saab", "Volvo", "BMW", "Banana", "Orange", "Apple", "Mango"];


// 获取数组长度

arrayName.length;


// 访问第一个数组元素

car[0];


// 访问最后一个数组元素

arr[arr.length -1];


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

## Array Enumeration

#### for

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


const fruits = ["Banana", "Orange", "Apple", "Mango"];

for(var i = 0; i < fruits.length; i++) {

    console.log(fruits[i]);
}


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### in

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


/**
 * 判断某个属性在对象中是否存在
 * 
 * 
 */

console.log('value' in obj): boolean;


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### for ... in ...

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


/**
 * for in 循环并不会枚举出 空值
 * 
 * 
 * for 循环可以循环空值( undefined )
 */

const fruits = ["Banana", "Orange", "Apple", "Mango"];

for(let prop in fruits) {

    console.log(fruits[i]);
}


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### for ... of ...

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


/**
 * for ... of ...
 * 
 * 
 * 可用于枚举可迭代对象
 */

const fruits = ["Banana", "Orange", "Apple", "Mango"];

for(let prop of fruits) {

    console.log(fruits[i]);
}


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### forEach();

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


fruits = ["Banana", "Orange", "Apple", "Mango"];

fruits.forEach(function (value, index, array) {

    console.log(value);

    // =>   Banana
    //      Orange
    //      Apple
    //      Mango

    console.log(index);

    // =>   0
    //      1
    //      2
    //      3


    console.log(array);

    // =>   Array(4)
    //      0 = "Banana"
    //      1 = "Orange"
    //      2 = "Apple"
    //      3 = "Mango"
    //      length = 4
    //      __proto__ = Array(0)

})


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```

#### Associative Array

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


/**
 * 许多编程语言都支持带有命名索引的数组, 具有命名索引的数组称为关联数组（或`哈希`）
 * 
 * 
 * JavaScript 并没有支持数组名为索引，数组始终使用编号索引
 * 
 * 如果使用命名索引，JavaScript 会将数组重新定义为 `类数组`
 */
 

var person = [];
person[0] = "John";
person[1] = "Doe";
person[2] = 46;

var x = person.length;     // person.length will return 3
var y = person[0];         // person[0] will return "John"


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
``` 

## isArray Methods

``` javascript
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/**
 * method 1
 * 
 * 
 * ECMAScript 5定义了一种新方法 Array.isArray();
 */


fruits = ["Banana", "Orange", "Apple", "Mango"];

Array.isArray(fruits);   // returns true


//-------------------------------------------------------------------------------------------------------------------//


/**
 * method 2
 * 
 * 
 * 创建自己的 isArray() 函数
 */

function isArray(myArray) {

  return myArray.constructor.toString().indexOf("Array") > -1;
}


//-------------------------------------------------------------------------------------------------------------------//


/**
 * method 3
 * 
 * 
 * instanceof 对象是否是由给定的构造函数创建运算符返回 true
 */


var fruits = ["Banana", "Orange", "Apple", "Mango"];

fruits instanceof Array;   // returns true


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```